...
d3 <- subset(d3, months >= 3)

d2 <- ddply(d3, c("AREA_NAME"), function(x) {
  if(nrow(x) >= 1) 
...
par(mar = c(1,1,0,0), cex = 0.7)
plot(nao$year, nao$nao, type = "l", axes = FALSE)
par(mgp = c(2, 0.6, 0))
axis(1, las = 1, tck = -0.05, padj = -0.4)
axis(2, las = 1, at = c(-4, 0, 4), tck = -0.05)
abline(h = 0, lty = 2)
