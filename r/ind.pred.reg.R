...
library(ggplot2)
p <- ggplot(d.cor.out, aes(y = pred.name, x = r, colour = r)) + geom_point() + facet_wrap(~region, scales = "free_y") + geom_segment(aes(l, pred.name, xend = u, yend = pred.name)) + geom_vline(xintercept = 0);ggsave(paste("../fig/predators-cor-lag-", i, ".pdf", sep = ""), width = 14, height = 10)
}
